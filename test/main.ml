  ...
  name >:: fun _ -> 
    assert_equal final_grid (
      let rec generation_loop remaining_generations grid = 
        ...
        | r when r <= 0 -> grid
        | _ -> let new_population = new_generation grid in 
...
let state_tests = [
    ...
    "after 4 generations, a dead grid remains dead."
    4
    empty_grid
    empty_grid;

    next_generation_test
    "a dead grid cannot produce a living grid on its own"
    empty_grid
    empty_grid;
...
(* clicking on [1,1] of a [3,3] grid with all dead cells turns [1,1] alive, and vice versa*)
